...
#include <iostream>
using namespace std;
P_TRawCompileUnit TRawCompileUnit::instance = nullptr;
P_TRawCompileUnitDao

TRawCompileUnit::TRawCompileUnit() {
}

TRawCompileUnit::~TRawCompileUnit() {
}

P_TRawCompileUnit TRawCompileUnit::getInstance() {
	if (instance == nullptr) {
		instance = P_TRawCompileUnit(new TRawCompileUnit);
	}
	return instance;
};
...
void TRawCompileUnit::insertList(vector<unordered_map<string, string>> &list) {
	if (list.size() == 0) {
		return;
	}
	MYSQL conn;
...
void TRawCompileUnit::queryList(unordered_map<string, string> &transfer_map, vector<unordered_map<string, string>> &result_list) {
...
void TRawCompileUnit::deleteRecord(unordered_map<string, string> &transfer_map) {
	MYSQL conn;
	MYSQL_ROW mysql_row;
	mysql_init(&conn);
