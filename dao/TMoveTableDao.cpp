...
P_TMoveTableDao TMoveTableDao::instance = nullptr;


TMoveTableDao::TMoveTableDao() {
}

TMoveTableDao::~TMoveTableDao() {
}

P_TMoveTableDao TMoveTableDao::getInstance() {
	if (instance == nullptr) {
		instance = P_TMoveTableDao(new TMoveTableDao);
	}
	return instance;
};
...
void TMoveTableDao::insertList(vector<unordered_map<string, string>> &list) {
	if (list.size() == 0) {
		return;
	}
	MYSQL conn;
	...
	if (mysql_real_connect(&conn, host.data(), user.data(), password.data(), db.data(), 0, NULL, CLIENT_FOUND_ROWS)) {
		...
		sql_os << "insert into t_move_table(md5,item_id,interal_id) ";
		sql_os << "values";
...
void TMoveTableDao::queryList(unordered_map<string, string> &transfer_map, vector<unordered_map<string, string>> &result_list) {
	...
	if (mysql_real_connect(&conn, host.data(), user.data(), password.data(), db.data(), 0, NULL, CLIENT_FOUND_ROWS)) {
		...
		ostringstream sql_os;
		string col[] = { "md5","item_id","interal_id" };
		...
		sql_os << "select  md5,item_id,interal_id from t_move_table ";
		sql_os << "where 1=1 ";
		if (transfer_map.find("md5") != transfer_map.end()) {
			sql_os << "and md5 =" << "'" << transfer_map["md5"] << "'" << " ";
		}
		sql_os << "order by item_id" << " ";
...
void TMoveTableDao::deleteRecord(unordered_map<string, string> &transfer_map) {
	MYSQL conn;
	MYSQL_ROW mysql_row;
	mysql_init(&conn);
	...
	if (mysql_real_connect(&conn, host.data(), user.data(), password.data(), db.data(), 0, NULL, CLIENT_FOUND_ROWS)) {
		...
		sql_os << "delete from t_move_table ";
		sql_os << "where 1=1 ";
		if (transfer_map.find("md5") != transfer_map.end()) {
			sql_os << "and md5 =" << "'" << transfer_map["md5"] << "'" << " ";
		}
