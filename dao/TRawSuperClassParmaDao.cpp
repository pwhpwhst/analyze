...
#include "TRawSuperClassParmaDao.h"
#pragma comment(lib,"libmysql.lib")
#include <mysql.h>
#include <sstream>
#include <iostream>
using namespace std;
P_TRawSuperClassParmaDao TRawSuperClassParmaDao::instance = nullptr;


TRawSuperClassParmaDao::TRawSuperClassParmaDao() {
}

TRawSuperClassParmaDao::~TRawSuperClassParmaDao() {
}

P_TRawSuperClassParmaDao TRawSuperClassParmaDao::getInstance() {
	if (instance == nullptr) {
		instance = P_TRawSuperClassParmaDao(new TRawSuperClassParmaDao);
	}
	return instance;
};
...
void TRawSuperClassParmaDao::insertList(vector<unordered_map<string, string>> &list) {
	if (list.size() == 0) {
		return;
	}
	MYSQL conn;
	...
	if (mysql_real_connect(&conn, host.data(), user.data(), password.data(), db.data(), 0, NULL, CLIENT_FOUND_ROWS)) {
		...
		sql_os << "insert into raw_super_class_parma(appName,fileId,subId,subId2,classType,name) ";
		sql_os << "values";
...
void TRawSuperClassParmaDao::queryList(unordered_map<string, string> &transfer_map, vector<unordered_map<string, string>> &result_list) {
	...
	if (mysql_real_connect(&conn, host.data(), user.data(), password.data(), db.data(), 0, NULL, CLIENT_FOUND_ROWS)) {
		...
		ostringstream sql_os;
		string col[] = { "appName","fileId","subId","subId2","classType","name" };

		unordered_map<string, int> col_map;
		for (int i1 = 0; i1 < 6; i1++) {
			col_map[col[i1]] = i1;
		}
		sql_os << "select  appName,fileId,subId,subId2,classType,name from raw_super_class_parma ";
		sql_os << "where 1=1 ";
...
void TRawSuperClassParmaDao::deleteRecord(unordered_map<string, string> &transfer_map) {
	MYSQL conn;
	MYSQL_ROW mysql_row;
	mysql_init(&conn);
	...
	if (mysql_real_connect(&conn, host.data(), user.data(), password.data(), db.data(), 0, NULL, CLIENT_FOUND_ROWS)) {
		...
		sql_os << "delete from raw_super_class_parma ";
		sql_os << "where 1=1 ";
		//if (transfer_map.find("md5") != transfer_map.end()) {
		//	sql_os << "and md5 =" << "'" << transfer_map["md5"] << "'" << " ";
		//}
